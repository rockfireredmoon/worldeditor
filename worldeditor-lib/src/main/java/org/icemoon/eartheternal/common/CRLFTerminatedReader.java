...
public class CRLFTerminatedReader extends Reader {
	...
	private final int EOF = -1, CR = 13, LF = 10;
	 ...
	 *         with CRLF, or null if the end of the stream has been reached,
	 *         possibly discarding some characters in a line not terminated with
	 *         CRLF.
	...
	public String readLine() throws IOException {
		...
		/*
		 * This boolean tells which state we are in, depending upon whether or
		 * not we got a CR in the preceding read().
		 */
			...
			if (!cr_just_received) {
				...
				case LF: // LF without a preceding CR
					if (tainted != -1) {
						int pos = tainted;
						tainted = -1;
						throw new TerminationException("\"bare\" CR or LF in data stream at " + line, pos, line);
					}
	...
	public class TerminationException extends IOException {
		private int where;
		public TerminationException(int where, long line) {
			super();
			this.where = where;
		}

		public TerminationException(String s, int where, long line) {
			super(s);
			this.where = where;
		}
